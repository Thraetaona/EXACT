...
(module $cpu
  ...
  (func $execute
      (call_indirect
        (call $fetch_byte) ;; Multi-byte OpCodes are handled within the OpCode functions themselves.
      )
  )
  ...
  ;; These will fetch the next instruction/data byte or word from memory
  (func $fetch_byte (result i32)
    (block (result i32)
      (call $ram.read8 (call $register.segment.get (global.get $CS)) (global.get $IP)) ;; BIU, fetching instructions from memory
      (call $step_ip (i32.const 1)) ;; Reading an 8-bit value from memory should increment $IP by one.
    )
  )
  (func $fetch_word (result i32)
    (block (result i32)
      (call $ram.read16 (call $register.segment.get (global.get $CS)) (global.get $IP))
      (call $step_ip (i32.const 2)) ;; Fetching a 16-Bit value should increment $IP by 2.
    )
  )
  ...
  ;; NOTE: The reason that we do not use the fixed i32 number '10' in
  ;; AAM and AAD is that they are two-byte instructions; Intel engineers
  ;; had cleverly re-used the second byte (which is "normally" 0x0A = 10)
  ;; to represent the usual 10 rather than using hard circuity.
  ;; However, "any" number be provided, and the instructions would still work.
  (func $0xd4 (; AAM ;)
    (call $register.general.set8
      (global.get $AH)
      (i32.div_u
